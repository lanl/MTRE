...
class mPLUG_Owl(LargeMultimodalModel):
    ...
    def get_p_true(self, image, prompt, return_dict=True): #NOTE THIS IMPLEMENTATION IS DEPENDANT ON HOW THE MODEL OUTPUT IS SETUP.
        prompt += ' '

        outputs = self._basic_forward(image, prompt, return_dict=True)
        
        logits = torch.cat(outputs['scores'], dim=0).cpu().numpy()
        target = self.tokenizer('A', return_tensors="pt")['input_ids'][-1]
        target = target[-1]
        # print(target)
        criterion = torch.nn.CrossEntropyLoss(ignore_index=0)

        loss = criterion(outputs['scores'][0].cpu(),target.unsqueeze(0).cpu())
