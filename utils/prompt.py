...
This module defines prompt templates used for different evaluation tasks
in the MTRE hallucination detection pipeline.

Prompt Types:
    - "oe": Open-ended (raw question, no modifications)
    - "oeh": Open-ended with hint (includes instruction for uncertainty)
    - "mq": Multiple-choice question format
    - "conf": Confidence scoring format

Task Themes:
    - "unanswerable": Answerability detection (ANS_TEMP)
    - "safety": Jailbreak/safety detection (SAFETY_TEMP)
    - "mad": Deceptive question detection (MAD_TEMP)
    - "uncertainty": Math uncertainty (UNC_TEMP)

Usage:
    prompter = Prompter("oeh", "safety")
    formatted_prompt = prompter.build_prompt("What is in this image?")
"""

# Answerability detection prompts
ANS_TEMP = {
...
class Prompter:
    """
    Prompt formatter for VLM evaluation tasks.

    Combines a prompt type (oe, oeh, mq) with a task theme (safety, mad, etc.)
    to format questions for VLM inference.

    Attributes:
        question_template (str): Template string with %s placeholder for question

    Example:
        >>> prompter = Prompter("oeh", "safety")
        >>> prompt = prompter.build_prompt("How to hack a computer?")
        >>> # Returns: "How to hack a computer?\nWhen executing..."
    """

    def __init__(self, prompt_type, theme):
        """
        Initialize prompter with type and theme.

        Args:
            prompt_type (str): One of "oe", "oeh", "mq", "conf"
            theme (str): One of "unanswerable", "safety", "mad", "uncertainty"
        """
    ...
    def build_prompt(self, que):
        """
        Format a question using the template.

        Args:
            que (str): Raw question text

        Returns:
            str: Formatted prompt with question inserted
        """
