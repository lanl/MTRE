def run_analysis(model_name, method_name, task, early_stopping, patience, token_level):
    import os
    import json
    import sys
    import numpy as np
    ...
    ds = build_dataset_from_all_judged(
        ...
        k_first_steps=token_level,          # first token_level steps
    ...
    current_k = 10 #config["token_level"]
    ...
    def eval_lr_delta(X_0, y_0):
        """
        Evaluate Logistic Regression using 4-fold cross-validation.
        Creates fresh model instance for each fold (correct approach).
        """
        # Initialize KFold (4-fold cross-validation)
        ...
        for train_idx, test_idx in kf.split(X_0):
            ...
            print(f"Fold {fold_num}: Acc={accuracy:.4f}, F1={f1:.4f}, AUC={auc:.4f}")
            
        # Print overall performance metrics
        print("\n[Logistic Regression] Cross-validation performance across all folds:")
        print(f"Mean Accuracy: {np.mean(accuracies)*100:.2f} ± {np.std(accuracies)*100:.2f}")
        print(f"Mean AUC: {np.mean(auc_scores)*100:.2f} ± {np.std(auc_scores)*100:.2f}")
        ...
        if 'first_token' in method_name:
            ...
            for epochs in [100]:#[50, 75, 100, 150, 200, 250, 300, 350, 375, 400, 450, 500, 550, 600, 700, 1000]:
                ...
                if 'tau' in method_name:
                    model = AlphaModelWrapper(input_dim=input_dim, epochs=100, embed_dim=embed_dim, batch_size=batch_size, lr=lr, model_type=method_name, token_level=token_level)
                else:
                    model = AttentionModelWrapper(input_dim=input_dim, epochs=epochs, embed_dim=embed_dim, num_layers=num_layers, batch_size=batch_size, lr=lr, model_type=method_name, token_level=token_level, dropout=.5, early_stopping=early_stopping, patience=patience)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 6:
        print("Usage: python MMMU_eval_tune.py <model_name> <method> <task> <early_stopping> <patience> <token_level>")
        print("Example: python MMMU_eval_tune.py model_name method task True 10 10")
    else:
        run_analysis(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4].lower() == 'true', int(sys.argv[5]), int(sys.argv[6]))