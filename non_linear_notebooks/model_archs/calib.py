...
class AttentionModel(nn.Module):
    def __init__(self, input_dim, embed_dim=128, num_heads=8, num_layers=3, dropout=0.1, token_level=10):
        """
...
def calib_model_train(input_dim, x_train, y_train, config_dict, folds=2):
    def fit_head_fn(loader, config_dict, seed=None):
        ...
        # Initialize model
        if 'log_reg' in config_dict['model_type']:
            model = LogisticRegressionModel(input_dim, **{k: config_dict[k] for k in ['embed_dim', 'num_heads', 'num_layers', 'dropout', 'token_level']}).to('cuda')
        else:
            print('Running attention')
            model = AttentionModel(input_dim, **{k: config_dict[k] for k in ['embed_dim', 'num_heads', 'num_layers', 'dropout', 'token_level']}).to('cuda')
    ...
    cal = SequentialReliabilityCalibrator(
        ...
        search = SearchConfig(
            ...
            Kmax_grid = list(range(0, config_dict.get('token_level'))),
            lam_grid=np.arange(0.0, .1, .1),
            metric="roc_auc",   # or try "roc_auc" if labels are well-balanced
            target_fpr=0.05       # or try [0.01, 0.10] for robustness
        ),
