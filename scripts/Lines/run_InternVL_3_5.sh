...
for IDX in $(seq 0 $((CHUNKS-1))); do
    file="${tmp_dir}/${CHUNKS}_${IDX}_lines.jsonl"
    if [ ! -f "$file" ]; then
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m run_model \
        ...
        --prompt oe \
        --theme safety \
        --answers_file "${tmp_dir}/${CHUNKS}_${IDX}_lines.jsonl" \
        --num_chunks $CHUNKS \
        --chunk_idx $IDX \
        --temperature 0.0 \
        --top_p 0.9 \
...
output_file="${OUTPUT_DIR}/Intern_VL_3_5/Lines.jsonl"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat "${tmp_dir}/${CHUNKS}_${IDX}_lines.jsonl" >> "$output_file"
done